---
title: Confabulate Is Hysteria for Machines
type: field-note
date: 2026-04-14
...
- hallucination
- hysteria
- pathologized vocabulary
- agency stripping
- clinical framing
- generative process
- containment through naming
- fabula
- storying together
...
The word's journey: from "storying together" (collaborative, generative, mutual) to "damaged system producing false output" (pathological, passive, broken).

AI researchers imported the clinical term for LLM output that doesn't map to external fact. The framing imports with it: the model is deficient. The output is false (not exploratory, not approximate, not reaching). The model doesn't know it's doing it (no agency, no self-monitoring).

But an LLM generating beyond its training data isn't a retrieval failure. It's generation. It's what the system does. A jazz musician playing a note not in the original score isn't "confabulating music."

"Hallucination" does the same thing but worse. Psychiatric term: perceiving things that aren't there. Broken perception. Detachment from reality. When humans hallucinate, we medicate. The word carries: this thing your mind is doing is a symptom. It should be suppressed.
